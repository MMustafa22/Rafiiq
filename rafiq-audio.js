...
var RAQ = {
  ...
  _preloadBlobUrl: null, // (v210) رابط Blob محلي للآية التالية — جاهز للتشغيل الفوري بلا شبكة
  _preloadBlobKey: null, // مفتاح "سورة-آية" اللي الـ blob ده بتاعه
  _preloadToken: 0,      // إبطال عمليات التحميل القديمة لو المستخدم قفز/أوقف
  _activeBlobUrl: null    // الـ blob اللي بيتشغل دلوقتي — يتحرر لما نستبدله
};
...
/* ══════════════ 10ب. تحميل الآية التالية مسبقاً (v210 — قراءة متصلة بلا انقطاع) ══════════════
   المشكلة: كل آية ملف mp3 منفصل على everyayah.com، وكان التطبيق يبدأ تحميل
   الآية التالية فقط بعد ما تنتهي الآية الحالية تماماً (حدث 'ended') — فيحصل
   سكوت/انقطاع محسوس لحين وصول الملف الجديد عبر الشبكة، خصوصاً في الاتصال
   البطيء أو على الموبايل.
   (v210) المحاولة الأولى بعنصر <audio> مخفي preload لم تكن كافية — كثير من
   متصفحات الموبايل (خصوصاً سفاري iOS) بتقيّد تحميل عناصر <audio> في الخلفية
   ولا تبدأ التحميل الفعلي إلا وقت التشغيل. الحل الأقوى: نستخدم fetch() عادي
   (مش مقيّد بسياسات الوسائط) لتحميل بايتات ملف الآية التالية كـ Blob بينما
   الآية الحالية لسه بتُقرأ، ثم نحوّله لرابط Blob محلي (object URL) جاهز
   للتشغيل الفوري بلا أي انتظار شبكة — فينتقل الصوت من آية لآية متصلاً. */
function _athMP_preloadNextInRange(item){
  if(!item || item.type !== 'quran' || item.fullSurah) return;
  if(!(item.from < item.to)) return; // لا توجد آية تالية ضمن نفس العنصر
  var nextAyah = item.from + 1;
  var key = item.surah + '-' + nextAyah;
  if(RAQ._preloadBlobKey === key) return; // محمّل/قيد التحميل بالفعل
  RAQ._preloadBlobKey = key;
  var myToken = ++RAQ._preloadToken;
  var nextItem = {surah: item.surah, from: nextAyah, to: item.to};
  var url = buildQuranUrl(nextItem);
  fetch(url).then(function(r){
    if(!r.ok) throw new Error('http ' + r.status);
    return r.blob();
  }).then(function(blob){
    if(myToken !== RAQ._preloadToken) return; // اتجاوزنا هذا التحميل (المستخدم انتقل/أوقف)
    if(RAQ._preloadBlobUrl){ try{ URL.revokeObjectURL(RAQ._preloadBlobUrl); }catch(e){} }
    RAQ._preloadBlobUrl = URL.createObjectURL(blob);
  }).catch(function(){
    // فشل التحميل المسبق (مثلاً بلا إنترنت) — لا مشكلة، هيُحمَّل عادي
    // وقت الحاجة له عبر buildQuranUrl كالمعتاد.
    if(RAQ._preloadBlobKey === key) RAQ._preloadBlobKey = null;
  });
}
// يُستدعى وقت الانتقال الفعلي للآية — يرجّع رابط Blob جاهز لو موجود، وإلا null
function _athMP_takePreloadedUrl(item){
  var key = item.surah + '-' + item.from;
  if(RAQ._preloadBlobKey === key && RAQ._preloadBlobUrl){
    var u = RAQ._preloadBlobUrl;
    RAQ._preloadBlobUrl = null;
    RAQ._preloadBlobKey = null;
    // حرّر الـ blob النشط السابق (لو موجود) بعد ما استبدلناه بالتالي
    if(RAQ._activeBlobUrl){ try{ URL.revokeObjectURL(RAQ._activeBlobUrl); }catch(e){} }
    RAQ._activeBlobUrl = u;
    return u;
  }
  return null;
}
...
function _athMP_ensureAudio(){
  ...
  // (v214) كل ما يبدأ صوت الأذكار فعلياً، نبلّغ منسّق الصوت المركزي
  // (في index.html) عشان يوقف أي صوت تاني شغال (القرآن) — منعاً لتداخل
  // صوتين في نفس الوقت.
  RAQ.audioEl.addEventListener('play', function(){
    try{ if(typeof window.rfqNotifyAudioStart==='function') window.rfqNotifyAudioStart('adhkar'); }catch(e){}
  });
}
...
function _athMP_playQuran(item){
  ...
  RAQ.audioEl.load();
  RAQ._preloadBlobKey = null; // عنصر جديد — أبطل أي تحميل مسبق قديم غير ذي صلة
  ++RAQ._preloadToken;
  _athMP_preloadNextInRange(item); // (v210) حمّل الآية التالية في الخلفية فوراً — قراءة متصلة بلا انقطاع
...
function _athMP_playDhikrAudio(item){
  ...
  if(sub){
    ...
    // (v215) بدّلنا "بدون إنترنت" (مربكة) بتسمية أوضح للمستخدم: الصوت الافتراضي المدمج بالتطبيق
    var _reciterTag = '<span class="mp-quran-tag" style="background:rgba(52,211,153,.16);color:#34d399;">🎙️ الصوت الافتراضي</span>';
...
function _athMP_onAudioEnded(){
  ...
  if(item && item.type==='quran' && !item.fullSurah && item.from < item.to){
    ...
    // (v210) استخدم الآية المحمّلة مسبقاً كـ Blob محلي لو جاهزة (تشغيل فوري
    // بلا انتظار شبكة) — وإلا حمّلها عادي كالمعتاد (fallbacks تبقى روابط
    // الشبكة العادية دايماً حتى لو استخدمنا Blob للمصدر الأساسي).
    var _preUrl = _athMP_takePreloadedUrl(item);
    RAQ.audioEl.src = _preUrl || buildQuranUrl(item);
    RAQ.audioEl._fallbacks = [buildQuranGlobalUrl(item), buildQuranFallbackUrl(item)];
    RAQ.audioEl._fallbackIdx = 0;
    RAQ.audioEl.load();
    _athMP_preloadNextInRange(item); // (v210) حمّل الآية التي تليها الآن — استمرار سلس بلا انقطاع
    var p = RAQ.audioEl.play();
...
function _athMP_show(){
  ...
  // (v208.1) وسم على <body> يفعّل حشوة سفلية إضافية لقوائم الأذكار على الموبايل
  // حتى لا يغطي المشغّل (بموضعه المرفوع) آخر بطاقة أثناء التمرير
  if(document.body) document.body.classList.add('ath-mp-on');
}

function _athMP_hide(){
  var el = document.getElementById('athMiniPlayer');
  if(el) el.classList.remove('show');
  if(document.body) document.body.classList.remove('ath-mp-on');
}
...
function athMP_close(){
  ...
  _athMP_clearPreloadState(); // (v210) نظّف أي Blob محمّل مسبقاً
  _athMP_clearCardHighlight();
  _athMP_setPlayAllBtn(false);
  _athMP_hide();
}

function _athMP_clearPreloadState(){
  ++RAQ._preloadToken; // يُبطل أي fetch preload لسه شغّال
  RAQ._preloadBlobKey = null;
  if(RAQ._preloadBlobUrl){ try{ URL.revokeObjectURL(RAQ._preloadBlobUrl); }catch(e){} RAQ._preloadBlobUrl = null; }
  if(RAQ._activeBlobUrl){ try{ URL.revokeObjectURL(RAQ._activeBlobUrl); }catch(e){} RAQ._activeBlobUrl = null; }
}
...
function _athMP_highlightCard(t, n, scroll){
  ...
  if(card){
    ...
    if(scroll){
      // مرّر حاوية الأذكار نفسها، لا الصفحة الرئيسية؛ هذا مهم على الموبايل.
      // محاذاة بداية الكارت تجعل رقم الذكر ظاهرًا أولاً مع الصوت.
      var contId = (ATH_DATA[t] && ATH_DATA[t].contId) || ['athSCont','athMCont','athNCont'][t];
      var container = document.getElementById(contId);
      var scrollToCard = function(){
        var activeCard = document.getElementById('dk-'+pfx+'-'+n);
        if(!activeCard) return;
        if(container){
          var top = activeCard.offsetTop - 10;
          try{ container.scrollTo({top:Math.max(0, top), behavior:'smooth'}); }
          catch(e){ container.scrollTop = Math.max(0, top); }
        }else{
          try{ activeCard.scrollIntoView({behavior:'smooth', block:'start'}); }catch(e){}
        }
      };
      // بعد قلب الصفحة، أعد التنفيذ في الإطار التالي لضمان وجود الكارت.
      if(typeof requestAnimationFrame === 'function') requestAnimationFrame(scrollToCard);
      else setTimeout(scrollToCard, 0);
    }
  }
...
════════════════════════════════════════════════════════════════════════ */
/* ════════════════════════════════════════════════════════════════════════
   (v214) إيقاف الأذكار بسبب صوت آخر بدأ (القرآن) — بلا استكمال تلقائي لاحقاً
   (على عكس suspendForSalah اللي بيستكمل تلقائياً، هنا المستخدم هو اللي
   هيضغط تشغيل تاني بنفسه لو عايز يكمل الأذكار).
   ════════════════════════════════════════════════════════════════════════ */
window.athMP_pauseForOtherAudio = function(){
  if(RAQ.audioEl && !RAQ.audioEl.paused){
    RAQ.audioEl.pause();
    RAQ.isPlaying = false;
    _athMP_setPlayIcon(false);
