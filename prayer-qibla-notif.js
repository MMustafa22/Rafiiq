...
(function() {
  ...
  function _pauseDhikrForSalah(reason) {
    ...
    _pauseQuranForSalah(reason); // (v214)
  }
  function _resumeDhikrForSalah(from) {
    ...
    _resumeQuranForSalah(from); // (v214)
  }
  // (v214) نفس تكامل الأذكار مع الأذان، لكن لصفحة القرآن الكريم (تعمل
  // داخل iframe منفصل quranFrame — نفس الأصل، فالوصول لدالتها مباشر وآمن).
  function _pauseQuranForSalah(reason) {
    try {
      var fr = document.getElementById('quranFrame');
      if (fr && fr.contentWindow && typeof fr.contentWindow.rfqQuranSuspendForSalah === 'function') {
        fr.contentWindow.rfqQuranSuspendForSalah(reason);
      }
    } catch (e) {}
  }
  function _resumeQuranForSalah(from) {
    try {
      var fr = document.getElementById('quranFrame');
      if (fr && fr.contentWindow && typeof fr.contentWindow.rfqQuranResumeAfterSalah === 'function') {
        fr.contentWindow.rfqQuranResumeAfterSalah(from);
      }
    } catch (e) {}
  }
  ...
  /* ============================================================
     ...
     ============================================================ */
  async function init() {
    // حماية من تكرار التهيئة إذا تم حقن الملف أو إعادة ربط DOM أكثر من مرة.
    ...
    if (state.hourlyRefreshInterval) clearInterval(state.hourlyRefreshInterval);
    state.hourlyRefreshInterval = setInterval(() => { if (!document.hidden) refreshPrayerTimes(); }, 60 * 60 * 1000);
    ...
    if (state.midnightRefreshTimer) clearTimeout(state.midnightRefreshTimer);
    state.midnightRefreshTimer = setTimeout(() => {
      if (!document.hidden) refreshPrayerTimes();
      if (state.dailyRefreshInterval) clearInterval(state.dailyRefreshInterval);
      state.dailyRefreshInterval = setInterval(() => { if (!document.hidden) refreshPrayerTimes(); }, 24 * 60 * 60 * 1000);
    }, msToMidnight);
  ...
  let _autoUpdateInitialized = false;
  ...
  function _initAutoUpdate() {
    ...
    if (_autoUpdateInitialized) return;
    _autoUpdateInitialized = true;
